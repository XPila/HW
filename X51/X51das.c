...
char* g_pcInsStrTab[X51_INS_MAX + 1] = {"???","ACALL","ADD","ADDC","AJMP","ANL","CJNE","CLR","CPL","DA","DEC","DIV","DJNZ","INC","JB","JBC","JC","JMP","JNB","JNC","JNZ","JZ","LCALL","LJMP","MOV","MOVC","MOVX","MUL","NOP","ORL","POP","PUSH","RET","RETI","RL","RLC","RR","RRC","SETB","SJMP","SUBB","SWAP","XCH","XCHD","XRL"};
	...
	else if ((cOp >= X51_OP_1) && (cOp < X51_OP_2))
	{
		...
		if ((cOp >= X51_OP_P0) && (cOp <= X51_OP_P7))
		{
			...
			case X51_OP_Bit:
				...
				{
					iVal = X51_BitAddrEnc(ucVal);
					if (pValue2Name) pcName = (*pValue2Name)(pParam, 'D', iVal >> 3);
					if (pcName) n = sprintf(pcAsm, "%s.%d", pcName, iVal & 7);
...
int X51Das_Ins(int iFlags, unsigned char* pucCode, unsigned short usAddr, char* pcAsm, int* piAsm, FX51Value2Name* pValue2Name, void* pParam)
{
	...
	if (psIns->op0) while (n <= 8) pcAsm[iAsm + (n++)] = ' ';
	iAsm += n;
	if (psIns->op0)
	{
...
int X51Das_ToFile(void* pFile, unsigned char* pucCode, int iAddrStart, int iAddrEnd, int iMaxInsCnt, FX51Value2Name* pValue2Name, void* pParam)
{
	...
	{
		int iAsm = 0;
		int iCnt = X51Das_Ins(X51_DAS_FLG_DISASM, pucCode, iAddr, cAsm, &iAsm, pValue2Name, pParam);
